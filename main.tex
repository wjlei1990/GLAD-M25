...
In the last few decades, we have witnessed great improvments in the global
seismic imaging. There are different types of earth models published using
various data types and methodologies\citep{}. The theory of Finite-frequcy
was brought up in the early 1980\citep{}, overcomes very limitations
in the ray-based theory. However, it is still the very recent
decade that people are able to use it in the global scale inverison, 
thanks to the advance in the high-performance computing and numerical methods.

There are gaps between the global and regional models. Regional models, tends to
resolve structures at shorter period bands such as 10 - 15 seconds\citep{},
but lacks of resolution in depth. 
On the other hand, global models, resolved structures as deep as 
the core-mantle boundary, but only starting from ~60 seconds period bands
at most, mainly due to the high computational cost\citep{}. Recent years
we have seen models moves to 30 seconds in waveform-type inversion but
there is still a clear gap between the global and regional inversions.
Owing to the recent advances in high-performance computing and numerical
methods, it is possible to simulate seismic waves in an unprecendental
resolution at global scale, to close the gap between regional and global
studies.

In our previous model paper\citep{}, advantanges of spetral-element methods
and adjoint tomography has been thoroughly discussed. Crust and discontinuity
is honored in our mesh. Thus, there is no crust correction needed in our
inversion procedures. Adjoint methods, can fully take the 3D earth structure
an attenuation into account, for both forward simulation and adjoint simulation,
calculating the frechet direvative in a more precise way.
However, when pushing for high frequecy, it soon becomes very
demanding on computational resourse. 

Other than the simulation part, the data assimilation has been another key factor
to the success of tomography. Ray-based travel-time inversion has accumulated
huge amount of data along it way. However, they usually focused on certain type of phases.
relying on both open and customized travel time database.
Models, such as GAP-P4 and UU-P07, used millions of travel time measurements, but only
used P related phases. 
The finite frequcy methods is developed recently so there are less consense of the dataset
used in the research. On the other hand, the methods also gives more freedom when it comes
to data assimilation. It then become a critial question for which part of the data should
be assimilated into the inverison. 
Recent years have seen a lot of good quality seismic networks deployed, such as US array.
However, there is few global model that take the advantage of network yet. It would be
a pity not to utlized it, but data assimilation should be handled very carefully since it
will directly impact the image in a few ways. First, it is the data quality. only good
seismograms and phases should be including into the inversion. Second, the high density
distribution of regional networks would skew the inversion so proper weightings should be
taken care of.

There are many challenges coming along the way. First, the data volume is huge.
There are in total of 1480 earthquakes used in the inversion stage, added into
the database step by step. Second, workflow management is crucial dealing with
such a large project. One factor is coming from the complex workflow. There are
4 major building blocks, forward simulation, seismic data processing, adjoint
simulation and post-processing. Each blocks contains several small blocks.
The other factor is coming from the number of earthquakes and high demands of
computation. In each iteration, we are doing 1480 forward and adjoint simulation,
generating a few Petabytes of wavefield files and consuming 16 million of CPU
hours on the supercomputer at Oak Ridge National Lab. There are yet more in
data processing stages. How to deal with hardware failures is very important
and prevent contamination into our inversion results.

In the following sections, we will present the GLAD-M25 earth model in details.
From section 2 - 4, we will focus on the data, including the starting model,
earthquakes and seismic data used in this study.  In section 5 and 6, we will
discuss the model parametrization and the choice of misfit function we made.
Section 7 and 8 describes the adjoint tomography workflow and the evolution
of inversion, and the improvments we made with the iterations. Section 9 will
focus on model evaluation part, which we used a few techniques to assess the
quality of inverison. Section 10 will address on presenting images of our model,
together with whole bunch of other models, global and regional. We conclude in 
section 10, summarizing our model and diretion of future works.
...
In addition to the 256 earthquakes used for the construction of first generation
model GLAD-M15, we initially selected 784 carefully chosen additional earthquakes
from the global Centroid-Moment Tensor (CMT) catalog~\citep[e.g.,][]{Ekstrom12},
...
one ASDF file contains all the information needed for data processing,
including the seismic traces, event information files (QuakeML), and station response files (StationXML).
...
The solver is GPU accelerated, and the simulations are performed on the Cray `Titan'
...
The figure nicely highlights all the well known body traveltime branches.
The maximum window count is 9,931 in the 134--135$^\circ$ distance and 1375--1376~s time bin.
...
we perform 1,480 forward and adjoint simulations, each involving hundreds of
...
For these reasons, we wish to harden the worflow by taking advantage of modern
workflow management software.
With this goal in mind, we selected RADICAL-SAGA and RADICAL-EnTK as our
workflow management engines, and developed complementary seismic tomography
workflow tools~\citep{EnTK2017}.
...
\begin{figure}
  ...
  each period band (rows) on three components (columns). (e)--(g) From GLAD-M18,
...
function resolution analysis as well as a ``held-out'' dataset, that is,
...
In lieu of such tests, we performed two point-spread function analyses~
\citep{fichtner2011resolution,zhu2015seismic,bozdaug2016global}.
...
in the actual inversion. This held-out dataset consists of all magnitude 6.3--7.0
...
compare global maps of our compressional wavespeed model with P models
...
190 earthquakes recorded by 745 seismographic station.
...
Beneath the Atlantic Ocean, our model strengthens slow anomalies beneath the
...
in GLAD-M25 compared to S372ANI+M, similar in shape to  model LLNL-G3Dv3.
...
Some plumes originate from the CMB and extend all the way to the Earth's surface.
...
\begin{figure}[h]
    \centering
    \sidesubfloat[]{\includegraphics[width=0.98\textwidth]{figures/plumes/Afar.png}\label{fig:plumes_a}}\\[-1pt]
    \sidesubfloat[]{\includegraphics[width=0.98\textwidth]{figures/plumes/Bermuda_Canary.png}\label{fig:plumes_b}}\\[-1pt]
    \sidesubfloat[]{\includegraphics[width=0.98\textwidth]{figures/plumes/CapeVerde_Hoggar.png}\label{fig:plumes_c}}\\[-1pt]
    \sidesubfloat[]{\includegraphics[width=0.98\textwidth]{figures/plumes/Iceland_Eifel.png}\label{fig:plumes_d}}\\[-1pt]
    \sidesubfloat[]{\includegraphics[width=0.98\textwidth]{figures/plumes/Easter_Galapagos.png}\label{fig:plumes_a}}\\[-1pt]
    \sidesubfloat[]{\includegraphics[width=0.98\textwidth]{figures/plumes/Marion3_Kerguelen.png}\label{fig:plumes_g}}\\
...
\begin{figure}[h]
    \centering
    \sidesubfloat[]{\includegraphics[width=0.88\textwidth]{figures/subductions/Aeg.png}\label{fig:subd_a}}\\[-1pt]
    \sidesubfloat[]{\includegraphics[width=0.88\textwidth]{figures/subductions/Br.png}\label{fig:subd_b}}\\[-1pt]
    \sidesubfloat[]{\includegraphics[width=0.88\textwidth]{figures/subductions/Ha.png}\label{fig:subd_c}}\\[-1pt]
    \sidesubfloat[]{\includegraphics[width=0.88\textwidth]{figures/subductions/Ne.png}\label{fig:subd_d}}\\[-1pt]
    \sidesubfloat[]{\includegraphics[width=0.88\textwidth]{figures/subductions/Wc.png}\label{fig:subd_e}}\\[-1pt]
...
Here, we presented our GLAD-M25 models. Starting from GLAD-M25, we
conducted 10 more iterations, with significantly more earthquakes
(up to 1480 earthquakes)and seismic data(up to 18.2 million time
windows) added in the inversion in a progressive way. It is the first
global transerse isotropoic model that brings the resolution down to
17 secs, but also invert the $V_p$ and $V_s$ indenpendently. Due to the
nature of spetral-element methos, our model meshes the crust and mantal
, without introducing the "crustal correction". Our solver also enables
us the take the full-attenuation into accout, both in forward and adjoint
simulations, using undo attenuation at the cost of more computation
hours and disk space.

GLAD-M25 is evaluated in many ways. First, it shows significant misfit
reduction in all measurements categories, but also in the variance
reduction in travel-time measurements. For the 360 held-out earthquakes,
the behavior is similar to what we have in our inversion dataset. We
presented our global $V_s$ and $V_p$, together with other well-known global
models and at large scale they agree with each other. However, when we focus
on regional studies and compare with regional models, our model shows superior
more detailed structures compared to other global models, staying at the 
same resolution of regional models. Our models shows great resolution in both
plumes and suduction zones, using the independently inverted $V_p$ and $V_s$
velocity models.
...
for their inversion. We culled seismograms from various data centers,
...
will be used but rejecting crap data.

However, global tomography research has progressed to a stage that not only relying
on the data and methodology, but also involving a lot of technique challenges
under the hood.
It has driven us to seek for the most cutting-edge technologies to solve them.
ADIOS was introduced to save mesh and kernel related data, since we are saving
petabytes of wavefiled snapshots in a few hours of simulation and it is critical
even for the most advanced supercomputers in the world.
ASDF is developed by us to store the seismograms since we have ten millions of
time series data from thousands of earthquakes. We have to get access and process
them in a very efficient manner that most of the existing data format just won't
meet our requirements.
To handle the complex inversion workflow, we are working RADICAL group to develop
the workflow management tools that will help manage every piece of task and make
the user aware if there is anything bad happened during the computation. Those tools that help the
...
For the future work, inversion for azimuthal anisotropy and attenuation is on the way.
The methodolody has already be demonstrated in a few regional studies, such as
\citep{}. We will start with our current dataset but increase our earthquakes to
more than $6,000$ in a progressive way , which will covered most of the
earthquakes in Global CMT project. We also plan to push our model resolution to
9 secs, with the Summit supercomputer available at Oak Ridge national lab. It will be
unprecedental in sesimology that pushes global tomography to such resolution, meanwhile
it will pose new chanllenges. Ensuring the reliability and stability on such a large machine
and associated file system, for example, will be one of those challenges. Even though the
computing powers has grown significantly, the storage has not. Our 17 sec has already
generated a few petabytes of wavefields files and 9 sec will generate at least one order
of magnitudes more files.

There are certains things that we would like to address on in the near future. First is the
the seismic data distribtuion. As we know, most of the seismic stations are distributed on
the continents, while the oceans are most left blank, with only a few stations on a few
islands. Floating arrays\citep{} might be promising in the future but it all comes down to the 
data processing and data quality. Second, harnessing such a large data set is difficult.
Artifical intelligence may be introduced and help us to make decisions when assimilating
measurements\citep{}.
...
{\color{red} Wenjie: Please add all relevant data centers.}
