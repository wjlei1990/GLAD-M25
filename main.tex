...
GLAD-M15~\citep{bozdaug2016global}, we present our second-generation transversely isotropic global model,
...
which is an updated version of model S362ANI~\citep{kustowski2008anisotropic},
...
of our database and greatly improve coverage in certain regions. \red{(Looks like there are also OBSs to be mentioned, mainly for my curiosity, it would be good to know their contribution to the overall inversions.)}
...
The Pyflex window selection algorithm is currently configured not to pick body waves after the arrival of the surface waves, which is why the PcP and ScS branches are missing at shorter epicentral distances \red{(Again just for my curiosity, why PcP and ScS phases together with any phase after surface waves were excluded? Any special reason or just a decision?)}.
...
\begin{table}
  ...
  \begin{tabular}{|c|c|c|c|}
    ...
    \hline
  \end{tabular}\\
  \caption{\small{Changes in fit in the twelve measurement categories
...
\begin{figure}
  ...
  the depth. \red{(Probably the numbers on top-left of each map are min/max perturbations of the associated sections, but the colorbar is fixed to 2 to 7$\%$)}}}
  \label{fig:global-vs}
\end{figure}
...
\begin{figure}[ht!] 
  ...
  \caption{\small{Map views of compressional wavespeed variations beneath South America for GLAD-M25~(first column) and global models S362ANI$+$M~\citep{moulik2014anisotropic} (second column), LLNL-G3Dv3~\citep{simmons2012llnl} (third column), and GAP-P4~\citep{fukao2013subducted} (fourth column).\red{(LLNL model looks pretty interesting with the craton signature down to 300 km. Slab and craton anomalies together at 150 km look interesting too, as if they are two different models. In addition I am still wondering why are we the only one seeing Scotia Arc? Or why the others do not see it? It looks to me that it is pretty easy to add a slab signature to Scotia because of USArray. If there are earthquakes in their database at Scotia it should still easy to put it there even without USArray. Or perhaps it is indeed the bias of USArray coming from GLAD-M15?.)}}}
...
\begin{figure}[ht!]
    ...
    (a) Afar; (b) Bermuda (left) and Canary (middle); (c) Cape Verde (middle) and Hoggar (right); (d) Iceland (middle) and Eifel (right); (e) Easter (left) and Galapagos (right); (f) Marion (middle), and Kerguelen (right). \red{(GLAD-M25 is still Vp)}}}
    \label{fig:plumes}
\end{figure}
...
All three models have common plume features where GLAD-M25 and TX2015 seem to be more in good agreement.  
...
There was a time when seismologists would meticulously inspect single seismograms to glean valuable information from them \red{(Perhaps it is now the case for planetary seismology? )}.
Today, we have easy access to data from thousands of earthquakes recorded by tens of thousands of seismographic stations, and individual inspection of all of these records has become impossible \red{(probably it depends on the seismologist. I remember Jeroen Ritsema telling that it is not a big deal to go through the entire database in a week or so with some not-so-sophisticated plotting techniques. Those interested in data still look at, I think..)}.
...
and to accommodate massive I/O during the postprocessing stage we use the ADIOS library~\citep{liu2014hello}.
For data assimilation,
tools such as FLEXWIN, which automatically select data windows suitable for measurements, are indispensable, and we see tremendous opportunities for the use of machine learning algorithms in this context~\citep[e.g.,][]{chen2017}.
...
To accomplish this next step, source parameters need to be constrained more carefully, especially as we push the resolution to shorter periods~\citep[e.g.,][]{valentine2010}.
...
\begin{figure}
  ...
  \caption{\small{1D radial-anisotropic shear and compressional wavespeed profiles for GLAD-M25 and STW105. The reference frequency for physical dispersion is ~7.77~mHz.}}
