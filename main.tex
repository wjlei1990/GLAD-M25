...
We observe that the 100~km diameter anomaly ---which is smaller than what was used in GLAD-M15--- at 300~km depth is well recovered, with little tradeoff between~$\beta_\mathrm{V}$ and~$\beta_\mathrm{H}$ and~$c$ (See SOM).
Figs.~\ref{fig:psf_tests}b--d shows the results of PSF tests for larger Gaussian anomalies (with 200~km to 400~km diameters) in the lower mantle beneath Afar,
