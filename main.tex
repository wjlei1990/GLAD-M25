...
generating a few Petabytes of wavefield files for the parsimonious-storage kernel calculation algorithm of~\cite{KoXiBoPeSaLiTr16}, and consuming 16~million OLCF node hours (\cyan{on Summit or Titan node hours? I think 1480 was gradually assimilated}).
...
the smallest moment magnitude in the database is set to~5.5 \cyan{5.5 is already the minimum period in globalcmt. Is it really the minimum? },
...
and the New Zealand National Seismograph Network~(NZ), \cyan{Japan?}
...
In this study,
it involves cross-correlation traveltimes \citep[e.g.,][]{luo_wave-equation_1991}
for body waves and frequency-dependent multitaper phase measurements for surface waves \citep[e.g.,][]{laske_constraints_1996} \cyan{no multitaper measurements for body waves?}.
...
so there are small tradeoffs between different measurement categories. \cyan{How were source corrections applied to GLAD-M15 and S362ANI+Crust2.0? Did you apply the correction terms computed for GLAD-M25 to all models?}
...
and Figs.~\ref{fig:psf_tests}a shows our result for the same location based on the current dataset of 1,480 events \cyan{Did you use all events in PSF tests or only a subset of them? I am asking just to confirm since I remember such a comment from Jeroen before}.
We observe that the 100~km diameter anomaly, which is smaller than that of used in GLAD-M15, at 300~km depth is well recovered, with little tradeoff between~$\beta_\mathrm{V}$ and~$\beta_\mathrm{H}$ and~$c$ (See SOM).
...
conduct full CMT inversions for the held-out database \cyan{not sure..}.
...
discussed in Appendix~\ref{sec:shanalysis} \cyan{but the models were not expanded to spherics harmonics to plot in this paper, right?}.
...
The 3D morphology of the African superplume shows a mantle plume rising from the core-mantle boundary with a broad base tilting northward \cyan{do you know why the African plume tilts? I think Pacific plume tilts likely because of the plate motion. GLAD-M25 generally shows that this tilt is happening widely in the upper mantle and perhaps the top of the lower mantle. But the African plume's tilt start from the CMB} with a thinning neck, before it reaches the 660~km phase boundary and flattens.
...
Subduction along the Andes in South America is imaged from south to north all the way down to the transition zone and beyond in a more continuous way compared to GLAD-M15, as shown in Fig.~\ref{fig:S_America}.
In the contour plot of positive shear-wavespeed perturbations we observe three gaps, which may correspond to ridges or volcanic plateaus on the subducted Nazca plate, and indicate a weakened or even torn slab \cyan{I think it is a great story but would you like to put the contour plot here or reserve for the future paper?}.
...
including Marion, Crozet, Kerguelen, Cocos, R\'eunion, and Seychelles.The visual inspection of the models show significant increase in perturbation anomalies in GLAD-M25 compared to GLAD-M15 that explains the misfit reductions in histograms (Figs.~\ref{fig:phase_hist}~\&~\ref{fig:phase_hist_360}).
...
the models share similar attributes. Relatively smoother slabs in GLAD-M25 are likely related to our choice of smoothing considering the minimum period of body waves in our simulations, $\sim17$~s, which is longer than typical teleseismic body-wave tomography studies.
...
GLAD-M25 is an elastic model with radial anisotropy confined to the upper mantle similar to its precessor GLAD-M15.
