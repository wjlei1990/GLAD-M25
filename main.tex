...
With the development of high performance computing and numerical methods,
it is now possible to using adjoint method to image the interior the globe
using thousands of earthquakes.

There are few agreements about the earth model and yet many unknown to be
resolved. There are quite a lot of global models exits however there is no such con-sense.
...
full advantages of parallel computing. In our workflow,
one ASDF file contains all the information needed for data processing,
including the seismic traces, event information files, and station response files.
...
iteration 21, prompting us to add more earthquakes and change the weighting strategy
again by reintroducing the 17--40~s surface waves.
...
earthquakes and an identical set of weightings and windows for three models,
...
The range of perturbations in shear wavespeed varies from map to map,
as indicated in the top left of each panel. Overall, these models are in good agreement
...
which are interpreted as back-arc extension associated with the roll-back.
...
In this section, we will visit the plumes systems on the earth. Fig.\ref{fig:plumes}
shows the cross section on several plumes. In addition, we also shows TX2015 model here.

Some of the plumes origins from the CMB and extend till the surface of the Earth,
even though their formation is still under heated discussion. Even though most of
the models tends to agree near the CMB, its rising shape diffs quite a lot in the
lower and upper mantle, thus dramatically changes the dynamics of the mantle
convections. Here, we present our model together with TX2015 and SEMUCB-WM1.

Our model shows that Afar plumes orignates from the CMB, narrows its diameter
in the middle mantle and broadens again above 660km discontinuity. It behaves
very similar to TX2015 while differs a lot with SEMUCB-WM1. Despite the similarity
with TX2015, our models shows stronger slow velocity anomaly around 660km
discontinuity, possibly indicating a enhanced partial melting at that depth. 

In fig.\ref{fig:plumes_b}, from left to right, we can see: 1) the Farallon
subduction slabs showing as fast anomaly; 2) the bermuda hotspot that stays
above 660km discontinuity; 3) Canary hotspot that originates from the CMT and
rise all the way up the the surface of the earth; 4) the Afar hotspot from another
angle of view.

In fig.\ref{fig:plumes_c}, from left to right, we can see: 1) the subduction slab
in South America showings as a big block of fast anomaly; 2) Cape Verde hotspot
that originates from the CMT and rise all the way up to the surface; 3) Hoggar hotspot
that has strong slow anomaly from 660km to the very surface. It is very interesting
to compare the image of Hoggar in our model with the other two, while the TX2015 only
shows slow anomaly around 660km and SEMUCB-WM1 only in the very surface.

In fig.\ref{fig:plumes_d}, plumes rises right below Icleland, reaches 660km and amplifies
its anomaly and rise to the surface. Here it may indicates another place where anmaly gets
much stronger above 660, possibly indicating the partial melting below 600km.

In fig.\ref{fig:plumes_e}, from left to right: 1) the Easter hotspot; 2) the Galapogos
hotspot; 3) the Central America subduction slabs showing as fast anomaly. It is interesting
seeing that the Easter and Galapagos may originates from the same underlying plumes, which
lies in the middle of both hotspots. The underlying plumes near the CMB belongs to the
a east extension arm of the Pacific super plume.

If fig.\ref{fig:plumes:f}, we are seeing Marion and Kerguelen hotspot, which might related
the Afar at its lower mantle part. Those hotspots and plumes are very hard to recover due
to the poor data coverage at the south hemisphere.
...
\begin{figure}[h]
    ...
    \caption{Vertical cross sections of perturbations of shear wave velocity Vs of plumes. (a) Afar; (b) Bermuda (left) and Canary (mid); (c) Cape Verde (mid) and Hoggar (right); (d) Iceland (mid) and Eifel (right); (e) Easter (left) and Galapagos (right); (f) Marion (mid) and Kerguelen (right). }
    \label{fig:plumes}
\end{figure}
...
\subsection{Subduction Slabs}
\label{section:slabs}

In this section, we visit another kind of geological systems on the earth,
subduction zones. In \cite{fukao2013subducted} and \cite{van2018atlas},
they both presented the subductions systems in the details. In
figure.\ref{fig:subd} we plotted our model, GLAD-M25, together with
GAP-P4 and UU-P07.

In general, all the models are in good agreement with each other, despite
the fact that our model use waveform measurements and the other two
uses travel-time. However, there are still difference when comparing those
models in details. At shallow depth, our models shows more consistent
structure in the continental crust and craton, across all the figures we
listed in fig.\ref{fig:subd}. When it comes to the upper and middle mantle,
all the models shares a a lot in common. Difference becomes obvious at depth,
especially for subductions below 1000km. For example, in fig.\ref{fig:subd_c},
it shows the Frallon subductions. Our models shows a more continous subductions
that goes all the way the lower mantle, while the other two travel-time models
only shows the subduction slab vaguely. This comes from two factors. Our model
incorporates more data, including the whole US-Array data for this region
specifically. Second, the finite-frequency kernels have more sensitivity in
depth, when compared to the classical ray-theory based inversions.

Also, as mentioned in the previous sections, we showed our model in Vp
perturbations, since in our inverison, Vp is treated as independant
parameters, while in previous waveform models, they usually fixed the
Vp and Vs ratios.
...
\begin{figure}[h]
    ...
    \caption{Vertical cross sections of perturbations of Vp in subduction regions. (a) the Aegean anomaly; (b) South American Subduction; (c) the Hatteras anomaly; (d) the Nepal Anomaly; (e) the Wichita anomaly.}
    \label{fig:subd}
\end{figure}

\section{Conclusion}

In summary, the new model performs well for the held-out dataset,
both in terms of misfit reductions and measurement distributions
in all twelve measurement categories. We expect to see similar
behavior for any future earthquakes.
...
\section{Window distributions}

Fig.\ref{fig:window_density} shows the window density map on the
vertical component for 17-40 sec period band. In this category,
we picked in total of 4,363,719 time windows. The color denotes the
density of windows. X axis is the time and y axis is the epicenter
distance. Windows are discretized into 1 degree in epicenter distance
domain and 1 second in time domain.

The max density value of windows is 9,931, located at 134-135 epicenter
distance bin and 1375-1376 time bin.

\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{figures/window_colorbar_linear_Z.pdf}
  \caption{The window density map.}
