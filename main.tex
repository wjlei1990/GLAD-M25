...
an almost six-fold increase over the previous generation model.
...
categories.
We performed similar assessments for a held-out dataset consisting of 380 earthquakes,
...
The first global ``adjoint tomography'' model of the Earth's mantle was published in 2016~\citep{bozdaug2016global}, more than 30 years after the development of the original ``full waveform inversion'' (FWI) theory.

Current global models are in general agreement regardless of data type and inversion strategy in terms of long-wavelength heterogeneity \citep{ritzwollerlavely1995,TW01,beckerboschi2002}.
...
SPECFEM3D\_GLOBE~\citep{KoTr02a,KoTr02b} on the Cray ``Titan'' at the Oak Ridge Leadership Computing Facility (OLCF) for global adjoint tomography.
...
The ultimate goal is to use anything and everything in seismograms,
a task made partly feasible based on the automated window selection tool FLEXWIN~\citep{maggi2009automated}.

Global adjoint tomography has a well-defined but complex workflow with multiple
stages involving numerical simulations of synthetic seismograms (forward simulations),
pre-processing of seismic data, making measurements, data assimilation, and Fr\'echet derivative
...
For these reasons, we use RADICAL-SAGA and RADICAL-EnTK as our
workflow management engines~\citep{EnTK2017}.
These workflow engines can automatically detected job failures,
...
EU60 is based on the assimilation of data from 190 regional earthquakes recorded by more than 700 European seismographic stations.
...
earthquake database, and the seismographic data set. 
...
The related Application Programming Interfaces (API)s are carefully designed for easy data extraction and parallel implementation.
...
Fig.~\ref{fig:window_density_Z} shows a time-distance plot
...
Fig.~\ref{fig:window_density_R} and Fig.~\ref{fig:window_density_T} shows for Radial and Transverse component in 17--40~s period bands.

\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{figures/window_colorbar_linear_R.pdf}
  \caption{Same as Fig.~\ref{fig:window_density_Z}, except for Radial component 17--40~s body waves.}
...
\begin{figure}
  ...
  \caption{Same as Fig.~\ref{fig:window_density_Z}, except for Transverse component 17--40~s body waves.}
...
Fig~\ref{fig:P_phase_hist} shows histograms of the phase measurements of first P
arrivals in two of the body wave measurement categories. All the
categories show consistent reduction in both mean and variance values. 
Although there are various phases assimilated and
P arrivals only contribute ~8\% of the overall measurements,
the histogram shows we are still doing a good job at minimizing the
misfit of the P arrivals.

\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{figures/dt_histogram_P.pdf}
  \caption{Histograms of phase measurements of P arrivals in two measurement categories for S362ANI combined with CRUST2.0 (M00, \textbf{Black}), GLAD-M15 (M15, \textbf{{\color{ForestGreen} Green}}) and GLAD-M25 (M25, \textbf{{\color{Red} Red}}). Other settings are the same as Fig.~\ref{fig:phase_hist}. }
...
%     \hline
%   \end{tabular}\\
%   \caption{Misfit Reduction from M00 to M25 for 360 earthquakes as held-out data set}
...
P phase measurements were also extracted and compared in Fig.~\ref{fig:phase_hist_360_P} for 3 models. The behavior is consistent as we expected.

\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{figures/dt_histogram_360_P.pdf}
  \caption{Histograms of phase measurements of P arrivals for the 360 events in the held-out database in Vertical and Radial 17--40~s period band .}
...
Vertical cross sections are plotted relative to the spherical average of MODEL-M25,
...
Lithosphere~\citep{muller2008age}.
...
and to accommodate I/O during postprocessing stage we use ADIOS~\citep{liu2014hello}.
...
because classical inversion workflows suffer from I/O inefficiencies, lack of fault tolerance, and an inability to work in distributed resource environments.
The RADICAL-SAGA and RADICAL-EnTK workflow management engines~\citep{EnTK2017} facilitate tracking of tasks and job failure detection,
enabling semi-automatic job resubmission if necessary.
...
The most recent machine at OLCF, called ``Summit'', an IBM Power9 accelerated by NVIDIA Tesla V100 GPUs,
can readily accommodate such simulations.
