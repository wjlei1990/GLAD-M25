...
significant improvments on all the categories over iterations.
The means values, has also improved expecially in the long period categories,
...
different compnents and period bands. To summarize, the improvements in
...
structures at serveal depths. As comparisons, we also showed two more global
...
** Note: add sphereical hormonica analysis later **
...
The deployment of USArray has dramatically incresed the data overage on the
...
US22 is radially anosotropic model build based on adjoint tomography using
USArray data from 180 regional earthquakes using 15-50sec short-period
body waves and 25-150sec long-period surface waves.
SL2013NA is an uppe-matnle global shear wave speed model using multimode
surface wavefrom tomography and primally focused on the north american
region.

The North American region is featured by a large, high-velocity cartonic
lithosphere, bounded by the Rocky Mountain Front to the west and continental
margein to the east. For both sides the lithosphere has been deformed by
low velocity structures thus form a sharp boundary.

At 100km, our model shows a much sharpened boundary between the carton
and surrounding low veclocity strcutures, especially compared with
...
The very high-velocity caraton extend to 200km, with its very core part to
300 km, similiar to US22 while SL2013NA shows it almost disappeared at such depth.
Again, the Wyonming craton and Mecicine Hat Block shows a very strong lithospheric
root from the very shallow depth to 400km, which is also smiliar to US22 but
hard to observe in SL2013NA. Similiar case is the craton at Yavapai and
Mazatzal Blocks, which define the souther bound of the large craton.

At such depth, we also observed high velocity
features within the Gulf of Mexico, which conincides spatially with the deepest
bathymetry and corresponds to a portion of anciet oceanic
Lithosphere\citep{muller2008}. The remanant of Juan de Fuca subudction
...
At the North, our models shows a extended high veloicty craton beneath the 
Greenland, though clearly seperated with the North America Craton,
bounded at the relativly low velcoity strcture at Barffin Bay and Labrador
...
In the nort easter part, the Iceland hotspot has been greatly enhanced,
...
Atlantic, starting from the tip of Florica to the New York,
seperated from the major part the north america craton,
...
Bermuda hotspot is nicely perserved from our starting
model, yet get sharpened and stronger at 200km and 300km. Besides, the
Carribean suduction, which is relatively young subduction systems, is clearly
...
zones. At 250km, our models shows the subudction in the east part.
...
Following the approch by \cite{tape2009adjoint}, \cite{bozdaug2016global},
...
the ditribution of 360 earthquake that we used as held-out dataset
...
is from M00 to M25. It agains shows misfit reduction in all categories in
our held-out dataset, simliar to what we observed from the inversion dataset.
...
Seismology is driven by data. Back into time, seismolgist would look at
single seimograms and pick valuable information out the them. Now, we have
ten thousands of seismograms deployed globallay and it will be a pity not
to take all the information we have. Eventhough GLAD-M25 is following-up
of GLAD-M15. It has many superior advantages and improvements. We increase
the dataset from 253 to 1480 earthquaks, with carefully inverted sources
...
thanks to the open environment and data sharing spirit of our seimology community.
With very careful checks by assessing the integrity of instrument response
and deployment, evaluating the singal-to-noise ratios, picking the time windows
using over 20 criterias, cleaning windows by their statistical behavior, and
calculating weights to balance the uneven distribution of earthquakes and
seismic stations, we really have done a lot to ensure the cleaness of the
data assimilated into our inversion and make sure every piece of value infomation
...
and kernel related data, since we are saving petabyes of wavefiled snapshots
...
inversion workflow, we are working RADICAL group to develop the workflow mangement
...
to make sure our scientific results away from contaminations. Besides, the
...
They are working closly with hardware vendors such as Cray, IBM and NVIDIA to adapt
