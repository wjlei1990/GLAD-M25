...
{\color{Red} To Jeroen: I checked the code. we are using $\omega_{cr}^{-1}$. The normalization factor depends on c.}
...
{\color{red} To Jeroen: You are right. $\Delta$ is different for a given source and category. Actually, we don't directly control $\Delta$. Instead, we control the condition number of weights. So the $\Delta$ will vary with receiver distributions. }
