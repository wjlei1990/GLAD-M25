...
\response{We have modified the language.\cyan{??}}
...
%The reason we mentioned the cost of PSF tests in the original manuscript was rather to make the point that despite the cost we will not get the full resolution. Otherwise, the core hours have never been an issue for our INCITE allocations but the ability to handle large data sets in FWI while carefully investigating the model evolution is the main challenge. 
Nevertheless, we added six PSF tests in the revised manuscript in addition to our existing quantitative comparisons of traveltime and amplitude misfits.
}
...
The statement ``their model does not tell us many new things about the internal structure of the Earth'' is again unnecessarily harsh and uncalled for. We already emphasize the advantages of our models, as we believe the reviewer knows well. 
For example,
...
\response{We have modified the manuscript substantially, as discussed previously. We focus more concretely on differences between M15 and M25 to better highlight the power of 10 additional iterations with $>1,000$ events.  
Unfortunately,
...
which limits simulations to periods up to $\sim250--300$~s.
}
...
where there full effects of attenuation, i.e., physical dispersion and dissipation, will be taken into account. We have some experience with such inversions in Europe (Zhu et al., 2013), and for problems in exploration seismology. We have also started experimenting at the global scale. The reviewer is right that the Q model will directly affect the traveltimes;
...
\response{Yes. M15 tried to compensate for this based on kernel density weighting, but the new approach demonstrably leads to faster and better convergence (see Ruan et al.~2019).}
...
\response{Points taken. PPP is not prominent, and picked rarely by Pyflex with conservative selection criteria to avoid bad-quality windows. It is ongoing research to improve automated phase picking algorithms.}
...
\response{Due to our cross-correlation measurements higher-modes may well be underrepresented in the current inversions. We are working on other misfit functions to effectively incorporate smaller-amplitude signals (e.g., Yuan et al. 2020) together with better window-selection strategies.}
